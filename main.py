...
app = FastAPI(
    ...
    description="AI-powered question generation for any subject using HuggingFace Transformers",
    version="1.0.0"
)
...
def generate_questions_from_text(text: str, max_questions: int = 10) -> List[str]:
    ...
    try:
        ...
        for i, sentence in enumerate(sentences):
            ...
            try:
                ...
                # The valhalla/t5-base-qg-hl model expects input in format: "generate question: <context>"
...
async def startup_event():
    ...
    logger.info("Starting QuestionCraft AI Backend...")
    success = load_model()
    if not success:
...
async def root():
    ...
    return HealthResponse(
        ...
        message="QuestionCraft AI Backend is running" if model_loaded else "Model not loaded"
    )
...
async def health_check():
    ...
    return HealthResponse(
        status="healthy" if model_loaded else "unhealthy",
        model_loaded=model_loaded,
        message="All systems operational" if model_loaded else "Model loading failed"
    )

@app.post("/generate-questions", response_model=QuestionResponse)
async def generate_questions(request: QuestionGenerationRequest):
    """Generate questions from input content"""
    import time
    ...
    try:
        ...
        if model is None or tokenizer is None:
            ...
            if not success:
                raise HTTPException(
                    status_code=500, 
                    detail="Failed to load AI model. Please try again later."
                )
